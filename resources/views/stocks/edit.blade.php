...
<body>
    ...
    <form action="{{ route('stocks.update', $stock->id) }}" method="POST">
        ...
        <div>
            ...
            <input type="number" id="ready_stock" name="ready_stock"
                value="{{ old('ready_stock', $stock->ready_stock) }}" min="0" required>
        </div>
        <div>
            <label for="in_process_stock">Stok Dalam Proses:</label>
            <input type="number" id="in_process_stock" name="in_process_stock"
                value="{{ old('in_process_stock', $stock->in_process_stock) }}" min="0" required>
        </div>
        <div>
            <label for="process_status">Keterangan Proses (Opsional):</label>
            <input type="text" id="process_status" name="process_status"
                value="{{ old('process_status', $stock->process_status) }}">
        </div>
        <div>
