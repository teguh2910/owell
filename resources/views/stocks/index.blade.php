...
<head>
    ...
    <style>
        ...
        .critical-stock { background-color: #f8d7da; color: #721c24; font-weight: bold; } /* Gaya untuk stok kritis */
    </style>
</head>
<body>
    ...
    <div style="margin-bottom: 20px;">
        ...
        <form action="{{ route('stocks.refresh.all') }}" method="POST" style="display: inline-block; margin-left: 10px;">
            @csrf
            <button type="submit" class="btn btn-primary" style="background-color: #6c757d;">Refresh Semua Status Stok</button>
        </form>
    </div>
            ...
            <tbody>
                @foreach ($stocks as $stock)
                    ...
                    <tr class="{{ $stock->is_critical ? 'critical-stock' : '' }}">
                        ...
                        <td class="btn-group">
                            <a href="{{ route('stocks.edit', $stock->id) }}" class="btn btn-warning">Edit/Update</a>
                            <form action="{{ route('stocks.destroy', $stock->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                @csrf
                                @method('DELETE')
