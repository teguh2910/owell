    ...
    <form action="{{ route('stocks.store') }}" method="POST">
        ...
        <div>
            ...
            @if ($rawMaterials->isEmpty())
                <p>Tidak ada Raw Material yang belum memiliki entri stok. Harap tambahkan Raw Material terlebih dahulu
                    atau edit stok yang sudah ada.</p>
        ...
        @if (!$rawMaterials->isEmpty())
            <div>
                ...
                <input type="number" id="ready_stock" name="ready_stock" value="{{ old('ready_stock', 0) }}"
                    min="0" required>
            </div>
            <div>
                <label for="in_process_stock">Stok Dalam Proses:</label>
                <input type="number" id="in_process_stock" name="in_process_stock"
                    value="{{ old('in_process_stock', 0) }}" min="0" required>
            </div>
            <div>
